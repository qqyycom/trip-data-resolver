...
function projectToTrajectory(points: TripRecord["points"]): TrajectoryPoint[] {
  ...
  for (const point of points ?? []) {
    ...
    const direction =
      rawDirection === undefined ? undefined : Number(rawDirection);
    const loctime = rawLoctime === undefined ? undefined : Number(rawLoctime);
    const timestamp =
      rawTimestamp === undefined ? undefined : Number(rawTimestamp);
...
export default function TripDetailClient({
  ...
  const [showStoredMapMatched, setShowStoredMapMatched] = useState(true);
  const [showStoredMapMatchedPoints, setShowStoredMapMatchedPoints] = useState(false);
  ...
  const [effectiveMapMatchingThreshold, setEffectiveMapMatchingThreshold] =
    useState(50);
  const [mapMatchingRadius, setMapMatchingRadius] = useState(25);
  const [effectiveMapMatchingRadius, setEffectiveMapMatchingRadius] =
    useState(25);
  const [mapMatchedResult, setMapMatchedResult] =
    useState<MapMatchingResult | null>(null);
  const [mapMatchingProgress, setMapMatchingProgress] = useState<{
    current: number;
    total: number;
  } | null>(null);
  ...
  const tolerance =
    toleranceOptions[toleranceIndex]?.value ?? toleranceOptions[2].value;

  const originalTrajectory = useMemo(
    () => projectToTrajectory(trip.points),
    [trip.points]
  );
  const storedMapMatchedTrajectory = useMemo(
    () => projectToTrajectory(trip.map_matched_points ?? []),
    [trip.map_matched_points]
  );
  ...
  const storedMapMatchedCoordinates = useMemo(
    () => trajectoryToCoordinates(storedMapMatchedTrajectory),
    [storedMapMatchedTrajectory]
  );
  ...
  const storedMapMatchedCoordinatesGcj = useMemo(
    () => wgs84PathToGcj(storedMapMatchedCoordinates),
    [storedMapMatchedCoordinates]
  );

  const runtimeMapMatchedCoordinates = useMemo(() => {
    if (!mapMatchedResult) {
      return [];
    }
  ...
  const runtimeMapMatchedCoordinatesGcj = useMemo(
    () => wgs84PathToGcj(runtimeMapMatchedCoordinates),
    [runtimeMapMatchedCoordinates]
  );

  const hasStoredMapMatched = storedMapMatchedTrajectory.length > 0;
  const hasRuntimeMapMatched = Boolean(mapMatchedResult?.trajectory.length);

  const stats = useMemo(() => {
    const hasRuntimeMatched =
      enableMapMatching && Boolean(mapMatchedResult?.trajectory.length);
    const finalTrajectory = hasRuntimeMatched
      ? mapMatchedResult!.trajectory
      : storedMapMatchedTrajectory.length
        ? storedMapMatchedTrajectory
        : simplifiedTrajectory;

    const mapMatchedCount = hasRuntimeMatched
      ? mapMatchedResult!.trajectory.length
      : storedMapMatchedTrajectory.length || undefined;
  ...
  const runtimeMapMatchedSegments =
    enableMapMatching && hasRuntimeMapMatched && mapMatchedResult?.segments
      ? mapMatchedResult.segments
      : null;
  const shouldShowRuntimePath =
    enableMapMatching && hasRuntimeMapMatched && showMapMatched;
  const shouldShowRuntimePoints =
    enableMapMatching && hasRuntimeMapMatched && showMapMatchedPoints;
  const shouldShowStoredPath = hasStoredMapMatched && showStoredMapMatched;
  const shouldShowStoredPoints =
    hasStoredMapMatched && showStoredMapMatched && showStoredMapMatchedPoints;
  ...
  return (
    <div className="min-h-screen bg-muted/10">
      <div className="mx-auto max-w-6xl px-4 py-8 space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              ...
              {formatTimestamp(trip.end_time)} · 历时：
        ...
        {hasStoredMapMatched && (
          <div className="rounded-md border border-orange-200/70 bg-orange-50 px-4 py-3 text-sm text-orange-700">
            已从 DynamoDB 载入 {storedMapMatchedTrajectory.length} 个
            map_matched_points，默认以橙色轨迹展示；仍可启用下方地图匹配重新纠偏。
          </div>
        )}
            ...
            <CardContent className="space-y-6">
              <section className="space-y-3">
                ...
                <Slider
                  ...
                  onValueChange={(value) =>
                    setToleranceIndex(value[0] ?? toleranceIndex)
                  }
                  min={0}
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    <p className="text-sm font-medium">
                      {mapProvider === "amap" ? "高德地图" : "Mapbox 地图"}
                    </p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setMapProvider(checked ? "amap" : "mapbox")
                      }
                    />
              ...
              <section className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    <Label
                      htmlFor="toggle-simplified-points"
                      className="text-sm"
                    >
                      显示抽稀采样点
                    </Label>
                  ...
                  {hasStoredMapMatched && (
                    <>
                      <div className="flex items-center justify-between">
                        <Label
                          htmlFor="toggle-stored-mapmatched"
                          className="text-sm text-orange-600"
                        >
                          显示预置匹配轨迹
                        </Label>
                        <Switch
                          id="toggle-stored-mapmatched"
                          checked={showStoredMapMatched}
                          onCheckedChange={setShowStoredMapMatched}
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        <Label
                          htmlFor="toggle-stored-mapmatched-points"
                          className="text-sm text-orange-600"
                        >
                          显示预置匹配采样点
                        </Label>
                        <Switch
                          id="toggle-stored-mapmatched-points"
                          checked={showStoredMapMatchedPoints}
                          onCheckedChange={setShowStoredMapMatchedPoints}
                          disabled={!showStoredMapMatched}
                        />
                      </div>
                    </>
                  )}
                ...
                <div className="space-y-3">
                  ...
                  {enableMapMatching && (
                    <div className="space-y-3 border-l-2 border-emerald-500 pl-4">
                      ...
                      <div className="flex items-center justify-between">
                        <Label
                          htmlFor="toggle-mapmatched-points"
                          className="text-sm"
                        >
                          显示匹配采样点
                        </Label>
                      ...
                      <div className="space-y-2">
                        <Label
                          htmlFor="map-matching-threshold"
                          className="text-sm"
                        >
                          置信度阈值（%）
                        </Label>
                        <Slider
                          ...
                          onValueChange={(value) =>
                            setMapMatchingThreshold(
                              value[0] ?? mapMatchingThreshold
                            )
                          }
                        />
                        <p className="text-xs text-muted-foreground">
                      ...
                      {mapMatchingProgress && (
                        <p className="text-xs text-muted-foreground">
                          匹配进度：{mapMatchingProgress.current}/
                          {mapMatchingProgress.total}
                        </p>
                      )}
                      {mapMatchingError && (
              ...
              <section className="space-y-3">
                ...
                <ul className="space-y-2 text-sm">
                  ...
                  <li>
                    抽稀点数：{stats.rdpSimplifiedCount ?? stats.finalCount}
                  </li>
                  {hasStoredMapMatched && (
                    <li>
                      预置匹配点数：{storedMapMatchedTrajectory.length}
                    </li>
                  )}
                  ...
                  {typeof trip.mileage === "number" && (
                    <li>里程：{(trip.mileage / 1000).toFixed(2)} km</li>
                  )}
              ...
              {originalCoordinates.length > 0 ? (
                mapProvider === "mapbox" ? (
                  <TripMap
                    ...
                    mapMatchedPath={runtimeMapMatchedCoordinates}
                    precomputedMapMatchedPath={storedMapMatchedCoordinates}
                    mapMatchedSegments={runtimeMapMatchedSegments}
                    showOriginal={showOriginal}
                    showSimplified={showSimplified}
                    showSimplifiedPoints={showSimplifiedPoints}
                    showPrecomputedMapMatched={shouldShowStoredPath}
                    showPrecomputedMapMatchedPoints={shouldShowStoredPoints}
                    showMapMatched={shouldShowRuntimePath}
                    showMapMatchedPoints={shouldShowRuntimePoints}
                  />
                ) : (
                  <TripMapAmap
                    ...
                    mapMatchedPath={runtimeMapMatchedCoordinatesGcj}
                    precomputedMapMatchedPath={storedMapMatchedCoordinatesGcj}
                    showOriginal={showOriginal}
                    showSimplified={showSimplified}
                    showSimplifiedPoints={showSimplifiedPoints}
                    showPrecomputedMapMatched={shouldShowStoredPath}
